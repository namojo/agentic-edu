...
- 오늘 4시간 로드맵(실습 A · 실습 B · 보너스 A1~A3)을 머릿속에 그립니다.

## 사전 확인

모듈 00의 환경 점검 체크리스트 8개가 전부 `✅`이면 준비 완료입니다. 아직 `❌`가 있으면 모듈 00으로 돌아가세요. 특히 Cursor 실행만 되면 이번 모듈은 진행할 수 있습니다 (Gemini 키는 모듈 07 실습 B에서 본격 사용).

## 오늘의 학습 규약 (하이브리드 환경)

본 강의는 **하이브리드 환경**라는 약속 위에서 진행됩니다. 이 약속을 머리에 새기고 출발하면 중간에 헷갈리지 않습니다.
...
- **실습 B (한·미 뉴스 멀티에이전트)**: 모듈 07에서 4개 Agent를 조합해 "반도체 수출 통제" 키워드로 한·미 매체 비교 브리프를 30분 안에 뽑아봅니다.
...
- 의도를 자연어로 내놓고, 초안을 받고, 감수해서 반영. 이 작은 루프를 반복하면 모듈 07 실습 B까지 갑니다.
...
- 규약(하이브리드 환경): Cursor Free + 내장 LLM, Gemini는 별도 Python.
...
- 규약(하이브리드 환경): Cursor Free + 내장 LLM, Gemini는 Python 터미널.
- 오늘 4시간 로드맵: M00 → M07. 실습 A(파일 관리)·실습 B(뉴스 멀티) 두 축.
