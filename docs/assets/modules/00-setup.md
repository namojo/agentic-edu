...
축하합니다 — 방금 **직접 LLM API를 코드로 호출**했습니다. 이 경험 하나가 오늘 모듈 05(단일 Agent) 보너스와 캡스톤 모듈 07에서 다시 빛을 발합니다.
...
💡 교실 20명이 동시에 실습하면 공유 아님에도 각자 한도 충분. 하지만 개인이 "재미로" 반복 실행하면 몇 분 안에 다 씁니다. 모듈 07 캡스톤을 위해 **아껴 쓰세요**.
