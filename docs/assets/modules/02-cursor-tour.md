...
💡 Plan 모드의 강점: 대규모 변경을 바로 실행하지 않고 "먼저 설계도를 본 뒤 승인"하는 안전장치입니다. 모듈 07 캡스톤에서 이 패턴을 다시 씁니다.
...
- **무엇을 기대**: 터미널에 월별 합계 표. "Cursor 안에서 한 일"과 "터미널에서 Gemini로 한 일"이 같은 결과를 냅니다 — 이게 오늘의 Plan C 감각입니다.
